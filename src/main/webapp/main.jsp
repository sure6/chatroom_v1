...
<script>
	...
	function set(selectPerson) {
		if (selectPerson != "${login.username}") {
		//采用name属性值来
	...
	function bg(){
	 var bgc=$('#bgcolor').val();
	 //alert(bgc);
	 $('#mainbody').css('background-color',bgc);
...
<body>
	...
	<table width="778" height="276" border="0" align="center"
		cellpadding="0" cellspacing="0" id="mainbody">
		<tr>
		<!--  bgcolor="#f6fded" -->
			<td width="165" valign="top" id="online"
				style="padding:5px">在线人员列表</td>
				<!--"  bgcolor="#FFFFFF"  -->
			<td width="613" height="200px" valign="top" background="images/main_bj.jpg
				style="padding:5px; ">
	...
	<table width="778" height="95" border="0" align="center"
		...
		<form action="" id="form1" name="form1" method="post">
			...
			<tr>
				...
				<td width="500" height="37" align="left"><input name="from" type="hidden"
					value="${login.username}">[${login.username}]对 <input
				...
				<select  id="bgcolor" onchange="bg()"> 
				     <option value="#f6fded" selected="selected">默认聊天背景</option>
				     <option value="#BFF8D6">清爽绿背景</option>
				     <option value="#9DE6F7">淡蓝背景</option>
				  </select>
				</td>
			...
			<tr>
				...
				<td width="500" align="left"><input name="content" type="text"
					size="70" id="waitsend"
					onKeyDown="if(event.keyCode==13 && event.ctrlKey){send();}">
					<input name="Submit2" type="button" class="btn_grey" value="发送"
					onClick="send()"></td>
				
				<td colspan="2" align="right"><input name="button_exit" type="button"
